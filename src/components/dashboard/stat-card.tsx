...
export default function StatCard({ title, value, icon, description, linkHref, variant = "default", trend }: StatCardProps) {
  ...
  const iconVariantClasses = {
    default: "text-primary bg-gradient-to-br from-primary/20 to-primary/10 p-3.5 rounded-xl shadow-lg ring-1 ring-primary/20 group-hover:scale-110 transition-transform duration-300",
    warning: "text-orange-600 dark:text-orange-400 bg-gradient-to-br from-orange-200/80 to-orange-100/60 dark:from-orange-900/60 dark:to-orange-900/40 p-3.5 rounded-xl shadow-lg ring-1 ring-orange-300/50 dark:ring-orange-800/50 group-hover:scale-110 transition-transform duration-300",
    destructive: "text-red-600 dark:text-red-400 bg-gradient-to-br from-red-200/80 to-red-100/60 dark:from-red-900/60 dark:to-red-900/40 p-3.5 rounded-xl shadow-lg ring-1 ring-red-300/50 dark:ring-red-800/50 group-hover:scale-110 transition-transform duration-300"
  }


  return (
    <Link href={linkHref}>
      <Card className={cn("cursor-pointer relative", variantClasses[variant])}>
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3 relative z-10">
          <CardTitle className="text-xs md:text-sm font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">{title}</CardTitle>
          <div className={iconVariantClasses[variant]}>{icon}</div>
        </CardHeader>
        <CardContent className="relative z-10">
          <div className="flex items-baseline justify-between">
            <div className="text-3xl md:text-4xl font-extrabold bg-gradient-to-r from-slate-900 to-slate-700 dark:from-slate-50 dark:to-slate-200 bg-clip-text text-transparent">{value}</div>
            {trend && (
              <div className={cn(
                "text-xs md:text-sm font-bold flex items-center gap-1 px-2 py-1 rounded-lg",
          ...
          {description && <p className="text-xs md:text-sm text-slate-600 dark:text-slate-400 mt-3 font-medium">{description}</p>}
        </CardContent>
      </Card>
    </Link>
  )
